document.addEventListener('DOMContentLoaded', function() {
  ...
  // Gestion du clic sur l'indicateur de panier
  bindCartIndicatorClick();
});
...
function bindCartIndicatorClick() {
...
function updateCartDisplay(data) {
  console.log('updateCartDisplay appelée avec:', data);
  
  // Mettre à jour le compteur dans la navbar
  const cartCount = document.querySelector('.cart-count');
  console.log('cartCount trouvé:', cartCount);
  ...
  if (cartCount) {
    ...
    if (cartIndicator) {
      ...
      if (data.cart_count > 0) {
        cartIndicator.style.border = '2px solid red';
        cartIndicator.style.backgroundColor = 'yellow';
        cartIndicator.style.color = 'black';
        cartIndicator.style.padding = '10px';
        cartIndicator.style.zIndex = '9999';
      }
      
      console.log('Indicateur affiché:', data.cart_count > 0 ? 'visible' : 'caché');
      console.log('Style de l\'indicateur:', cartIndicator.style.display);
    } else {
      console.error('cart-indicator non trouvé!');
    }
  } else {
    console.error('cartCount non trouvé!');
  }
